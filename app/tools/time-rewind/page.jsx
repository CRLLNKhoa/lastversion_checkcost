...
import OutputTime from "@/components/layouts/output-time";
...
function TimeRWPage() {
  ...
  const handleSearch = () => {
    ...
    const tempArray = data.filter((item) => {
      return Number(item.day) >= inputValueCurrent && Number(item.day) <= inputValuePlaned;
    });
  ...
  const handleSortCost = () => {
    if(isSortCost){
      setResult(result.sort((a,b) => a.day - b.day));
      setIsSortCost(!isSortCost)
    }else {
      setResult(result.sort((a,b) => b.day - a.day));
      setIsSortCost(!isSortCost)
    }
